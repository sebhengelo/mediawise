...
add_action( 'widgets_init', 'competentie_stats_widget' );

function competentie_stats_widget() {
    register_widget('competentie_stats');
}

class competentie_stats extends WP_Widget {

    /** constructor -- name this the same as the class above */
    function competentie_stats() {
    $widget_ops = array( 'classname' => 'competentie_stats', 'description' => __('Simple stats scores for students', 'wplms-dashboard') );
    $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'competentie_stats' );
    $this->WP_Widget( 'competentie_stats', __(' Competentie Stats', 'wplms-dashboard'), $widget_ops, $control_ops );
    wp_enqueue_script( 'wplms-student-simple-stats', WPLMS_DASHBOARD_URL.'../../../plugins/wplms-dashboard/js/jquery.sparkline.min.js',array('jquery'),true);
  }
    ...
    echo '<div class="'.$width.'">
        ...
        switch($stats){
          case 'courses':
            ...
            ",$user_id)); // Aan de query toevoegen: categorie
        if(is_array($marks)){
          ...
          if ( $title )
            ...
            $label = __('Course','wplms-dashboard');
          $value = ((count($marks) / 15) * 100); // 15 vervangen met totaal aantal posts binnen categorie
          $value = (int)$value; // Decimalen weggehaald, maar hij rondt standaard naar beneden af: FIXEN
        ...
        if(is_array($marks)){
          ...
          $label = __('Units','wplms-dashboard');
        ...
        echo '<figure class="knob animate zoom" style="width:210px;min-height:210px;">
                    <input class="dial" data-skin="knob" data-value="'.$value.'" data-fgColor="#efefef" data-bgColor="#03A9F4" data-height="200" data-inputColor="#03A9F4" data-width="200" data-thickness="0.2" value="'.$value.'" data-readOnly=true />
                        <div class="knob_content"><h3 style="color:#03A9F4;">voltooid</h3></div>
                  </figure>';
